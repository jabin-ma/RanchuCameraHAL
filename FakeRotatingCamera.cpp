...
abc3d::AutoTexture loadTestPatternTexture() {
    std::string valueStr =
        base::GetProperty("vendor.qemu.FakeRotatingCamera.scene", "colors");
    if (valueStr.empty()) {
        valueStr =
...
bool FakeRotatingCamera::renderIntoRGBA(const StreamInfo& si,
    ...
    const auto gb = sp<GraphicBuffer>::make(
        ...
        static_cast<uint64_t>(si.usage), 0);
...
bool FakeRotatingCamera::readSensors(SensorValues* vals) {
    ...
    vals->rotation[0] = -1.0f;
    ALOGE("mjp readSensors rotation:%f %f %f",vals->rotation[0],vals->rotation[1],vals->rotation[2]);
    ALOGE("mjp readSensors accel:%f %f %f",vals->accel[0],vals->accel[1],vals->accel[2]);
    ALOGE("mjp readSensors magnetic:%f %f %f",vals->magnetic[0],vals->magnetic[1],vals->magnetic[2]);
    return true;
}
